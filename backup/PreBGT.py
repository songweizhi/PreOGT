...
def PreBGT(args):
    ...
    genome_cate_file    = args['genome_cate_file']
    aa_group_list       = args['aa_group_list']
    genomic_traits_file = args['genomic_traits_file']
    prepare_traits_file = args['prepare_traits_file']
    plot_traits         = args['plot_traits']
    ...
    genomic_traits_file_path, genomic_traits_file_basename, genomic_traits_file_extension = sep_path_basename_ext(genomic_traits_file)

    genomic_traits_OneCol               = '%s/%s_OneCol.csv'                % (genomic_traits_file_path, genomic_traits_file_basename)
    genomic_traits_plot_scatter_matrix  = '%s/%s_scatter_matrix.pdf'        % (genomic_traits_file_path, genomic_traits_file_basename)
    genomic_traits_plot_grouped_boxplot = '%s/%s_grouped_boxplot.pdf'       % (genomic_traits_file_path, genomic_traits_file_basename)
    ml_plot_algorithm_comparison        = '%s/%s_algorithm_comparison.pdf'  % (genomic_traits_file_path, genomic_traits_file_basename)
    ...
    #traits_matrix_train, traits_matrix_validation, genome_cate_train, genome_cate_validation = train_test_split(traits_matrix, genome_cate, test_size=test_size)
    ...
    for model_name, model in models:
        ...
        cv_results = cross_val_score(model, traits_matrix_train, genome_cate_train, cv=KFold(n_splits=10), scoring='accuracy')
    ...
    print('traits_matrix_validation')
    print(traits_matrix_validation)
...
LogisticRegression          0.08333333333333333  !!!
...
if __name__ == '__main__':
    ...
    wd                      = '/Users/songweizhi/Documents/Research/PreTR_ML'
    ...
    aa_group_list           = ['RED', 'IVYWREL', 'NQ_NQED']
    prepare_traits_file     = True  # True or False
    plot_traits             = True  # True or False

    args = {'faa_folder'            : faa_folder,
            'genome_cate_file'      : genome_cate_file,
            'aa_group_list'         : aa_group_list,
            'genomic_traits_file'   : genomic_traits_file,
            'prepare_traits_file'   : prepare_traits_file,
            'plot_traits'           : plot_traits}
